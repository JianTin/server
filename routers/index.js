...
router.get('/getCatrgoryList', function() {
...
router.get('/createUser', async (req, res) => {
  const { name, pwd, list, avatar } = req.query;
  console.log(avatar);
  const result = await UserModel.create({ name, pwd, list, avatar });
  res.json({
    status: 0,
    data: result
  });
...
router.get('/getToday', function(req, res) {
  const data = require('../data/List/today.json');
  res.send({ code: 0, data });
});
//返回每天数据
router.get('/getVis', function(req, res) {
  const data = require('../data/List/vis.json');
  res.send({ code: 0, data });
});
// 返回brand 数据
router.get('/brandDates', function(req, res) {
  const data = require('../data/List/brand.json');
  res.send({ code: 0, data });
});
// 返回活动数据
router.get('/getaAtivityList', function(req, res) {
  // res.set('access-control-all-origin', '*');
  setTimeout(function() {
    const data = require('../data/List/ActivitiesList.json');
    res.send({ code: 0, data });
  }, 300);
});
// 进行登录
router.get('/login', async function(req, res) {
  ...
  // if (captcha !== req.session.captcha) {
  //   return res.send({ code: 1, data: '验证码不正确' });
  // }
  ...
  if ((name, pwd)) {
    UserModel.findOne({ name })
      ...
      .catch(() => {
        res.send({ code: 1, data: '输入正确的用户名与密码' });
      });
  } else {
    res.send({ code: 1, data: '需要进行注册' });
  }
});
// 自动登录
...
// 第二个项目
// -----------------------------------
// 返回 会员的全部数据， 不止是 black的会员 【懒惰
router.get('/getBlackRights', function(req, res) {
  console.log('5555');
  const data = require('../data/member/member.json');
  res.send({ code: 0, data });
});
// 返回 课程
router.get('/getCouse', function(req, res) {
  const data = require('../data/course/course.json');
  res.send({ code: 0, data });
});
// 返回杂志信息
router.get('/getReads', function(req, res) {
  const data = require('../data/Magazine/data.json');
  res.send({ code: 0, data });
});
